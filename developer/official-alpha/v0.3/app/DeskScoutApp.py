...
class App(XamlApplication):
	...
	def initSettingsPage(self,root=None):
		...
		Settings.alarms.uls.enabled.Click += saveAll
		...
		Settings.alarms.low.enabled.Click += saveAll
		...
		Settings.alarms.high.enabled.Click += saveAll
		...
		Settings.alarms.rf.enabled.Click += saveAll
		...
		Settings.alarms.ff.enabled.Click += saveAll
	...
	def initHistoryPage(self):
		...
		def pushData():
			...
			for i in dates:
				...
				item = ComboBoxItem()
				item.Content = i
				if f"{tx.tm_mon}-{tx.tm_mday}-{tx.tm_year}" == i:
					item.IsSelected = True

				date_picker.Items.Append(item)
			for x in records:
				tx = time.localtime(float(x.time / 1000))
				date_picker.Text = f"{tx.tm_mon}-{tx.tm_mday}-{tx.tm_year}"
				avg += x.value
		...
		def loadData(date):
			...
			for x in rec1:
				tx = time.localtime(float(x.time / 1000))
				if int(x.time/1000) in range(date,date+86400):
					...
					if (tx.tm_year,tx.tm_yday,tx.tm_hour,tx.tm_min) in seen:
						...
						final.append(x)
				if last != (tx.tm_yday,tx.tm_year) and not (tx.tm_yday,tx.tm_year) in dseen:
					dates.append(f"{tx.tm_mon}-{tx.tm_mday}-{tx.tm_year}")
					last = (tx.tm_yday,tx.tm_year)
					dseen.append((tx.tm_yday,tx.tm_year))
	...
	def NavChangeSelect(self,sender,args):
		...
		if args.SelectedItem:
			...
			if item.Tag.as_(str) == "Settings":
				...
				self.transitionElementContent(self.document,XamlReader().Load(open("../assets/ui/loading.xaml", "r", encoding='utf-8').read()),lambda a=self:exec('a.page = "history"\na.lsc = -1\na.initHistoryPage()'))
