...
class GlucoseDataProvider():
	...
	def login(self,username,password):
		...
		resp = requests.post('https://share.dexcom.jp/ShareWebServices/Services/General/AuthenticatePublisherAccount',json={"accountName":username,'password':password,'applicationId':"d89443d2-327c-4a6f-89e5-496bbb0317db"})
		print("We have liftoff")
		print(resp.status_code,resp.text)
		if resp.status_code == 200:
			resp = requests.post('https://share.dexcom.jp/ShareWebServices/Services/General/LoginPublisherAccountById',json={"accountId":eval(resp.text),'password':password,'applicationId':"d89443d2-327c-4a6f-89e5-496bbb0317db"})
			if resp.status_code == 200:
	...
	def getState(self):
		if time.time()-self.last2 > 60:
			try:
				resp = requests.get("https://share.dexcom.jp/ShareWebServices/Services/General/AuthenticatePublisherAccount")
	...
	def _getAllReadings(self):
		...
		#print("AUTHSTAT",self.authstate,self.token)
		if self.authstate == sdk.AuthenticationState.AUTHED:
			resp = requests.post("https://share.dexcom.jp/ShareWebServices/Services/Publisher/ReadPublisherLatestGlucoseValues",json={'sessionId':self.token,"minutes":int(86400/60),"maxCount":2880})
